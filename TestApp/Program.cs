...
namespace TestApp
{
    ...
    public class Program
    {
        ...
        {
            var passed = true;
            var testVectors = global::Argon2TestVector.Test.Argon2Vectors;
            foreach (var (i, testVector) in testVectors.Select((a, i) => (i, a)))
            {
                passed = passed && TestVectorBasicCheck(i, testVector);
            }

            var res = passed ? "Argon2AgainstReference Passed" : "Argon2AgainstReference FAILED";
            Console.WriteLine(res);
            return res;
        }
