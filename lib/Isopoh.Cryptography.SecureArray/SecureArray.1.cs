...
public sealed class SecureArray<T> : SecureArray, IDisposable
{
    ...
    {
        return lockMemory switch
        {
            LockMemoryPolicy.None => new SecureArray<T>(size, SecureArrayType.ZeroedAndPinned, secureArrayCall),
            LockMemoryPolicy.BestEffort => Best(size, secureArrayCall),
            _ => new SecureArray<T>(size, SecureArrayType.ZeroedPinnedAndNoSwap, secureArrayCall),
        };
    }
