...
{
    private static void Store32(Span<byte> buf, uint value)
    {
        buf[0] = (byte)value;
        buf[1] = (byte)(value >> 8);
        buf[2] = (byte)(value >> 16);
    ...
    private static void Store32(Span<byte> buf, int value)
    {
        buf[0] = (byte)value;
    ...
    private static void Store32(Span<byte> buf, int offset, int value)
    {
    ...
    private static void Store64(Span<byte> buf, int offset, ulong value)
    {
        buf[0 + offset] = (byte)value;
    ...
    private static void StoreBlock(Span<byte> buf, BlockValues blockValues)
    {
    ...
    private static ulong Load64(Span<byte> value, int offset)
    {
        return value[offset]
    ...
    private static void LoadBlock(BlockValues dst, Span<byte> src)
    {
        for (var i = 0; i < QwordsInBlock; ++i)
        {
